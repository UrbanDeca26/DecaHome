...
(() => {
  ...
  function applySettingsToPublicUI() {
    ...
    const heroTitle = document.querySelector('.hero-copy h1') || document.querySelector('.hero-copy h2');
  ...
  function openGuideTopic(topic) {
    ...
    const detail = $$('#guideGrid details').find((node) => {
      const heading = node.querySelector('h3');
      return heading && heading.textContent.trim().toLowerCase() === String(topic).trim().toLowerCase();
    });

    if (detail) {
      detail.open = true;
      setTimeout(() => detail.scrollIntoView({ behavior: 'smooth', block: 'center' }), 120);
    }
  }
  ...
  function renderChatSuggestions(items = LUNA_DEFAULT_SUGGESTIONS) {
    ...
    wrap.innerHTML = list.map((item) => {
      ...
      return `<button type="button" class="chat-suggestion" data-chat-suggestion="${escapeHtml(query)}">${escapeHtml(label)}</button>`;
    }).join('');
  }

  function openInquiryForm(prefillMessage = '') {
    const panel = $('#chatInquiryPanel');
    if (!panel) return;
    panel.classList.remove('hidden');
    $('#chatInquiryMessage') && ($('#chatInquiryMessage').value = prefillMessage || $('#chatInquiryMessage').value || '');
    const name = $('#chatInquiryName');
  ...
  function populateQuickActionMessage(query) {
  ...
  function updateChatFromApi(data, fallbackMessage) {
    ...
    } else if (!String(data?.intent || '').toLowerCase().includes('escalate')) {
      closeInquiryForm();
    }
  ...
  async function sendChat() {
    ...
    pushChat('user', text);
    pushChat('bot', 'Luna is checking the site...');
    ...
    try {
      ...
      const reply = updateChatFromApi(data, text);
      if (thinking) thinking.textContent = reply;
      else pushChat('bot', reply);
    } catch (_) {
      const reply = localConciergeAnswer(text);
      renderChatSuggestions(LUNA_DEFAULT_SUGGESTIONS);
      if (/inquiry/.test(reply.toLowerCase())) openInquiryForm(text);
      else closeInquiryForm();
      if (thinking) thinking.textContent = reply;
      else pushChat('bot', reply);
    }
  }

  async function submitChatInquiry(e) {
    e.preventDefault();

    const payload = {
      name: String($('#chatInquiryName')?.value || '').trim(),
      email: String($('#chatInquiryEmail')?.value || '').trim(),
      phone: String($('#chatInquiryPhone')?.value || '').trim(),
      message: String($('#chatInquiryMessage')?.value || '').trim(),
      page: 'chat',
      topic: String($('#chatInput')?.value || '').trim() || 'General inquiry',
    };

    const btn = $('#chatInquirySend');
    ...
    try {
      ...
      $('#chatInquiryForm')?.reset();
      alert('Your inquiry has been sent to the host.');
    } catch (err) {
...
If you are testing locally, make sure the /api/book route is available and SMTP variables are set on deployment.`);
  ...
  function bindPublicEvents() {
    ...
    $('#chatQuickActions')?.addEventListener('click', (e) => {
      const btn = e.target.closest('[data-chat-suggestion], [data-query], [data-action]');
      if (!btn) return;

      const action = String(btn.dataset.action || '').trim();
      if (action === 'contact') {
        openInquiryForm(String($('#chatInput')?.value || ''));
        return;
      }

      const query = String(btn.dataset.query || btn.dataset.chatSuggestion || '').trim();
      if (query) {
        populateQuickActionMessage(query);
        sendChat();
      }
    });
    ...
    $('#chatSuggestions')?.addEventListener('click', (e) => {
      ...
      if (!btn) return;
      const query = String(btn.dataset.chatSuggestion || '').trim();
      if (query) {
        populateQuickActionMessage(query);
        sendChat();
      }
    ...
    $('#closeInquiryPanel')?.addEventListener('click', closeInquiryForm);
    $('#chatInquiryForm')?.addEventListener('submit', submitChatInquiry);
  ...
  window.addEventListener('error', (event) => {
    console.error('Luxury Stay app error:', event.error || event.message || event);
  });
